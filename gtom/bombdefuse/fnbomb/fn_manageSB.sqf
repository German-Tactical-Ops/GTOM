...
// Example: [_sb, _iedside, _trapvolume, _IEDExplosionType] spawn GTOM_fnc_manageSB;
...
// <out>	NOTHING
//==================================================================================================================================================================
#define	GTOM_MANAGESB_IED "DemoCharge_Remote_Ammo"
...
_dummy = GTOM_dummy createVehicle (getpos _sb);
_init = '_this hideobject true;';
[[[netid _dummy,_dummy], _init], "GTOM_fnc_setVehicleInit", true, true] spawn BIS_fnc_MP;
...

			{
...

				{
					if(_enemy isKindOf _x && ({alive _x} count (crew _enemy) > 0) && _check) then {
						...
						private ["_exp"];
						{
							_exp = GTOM_MANAGESB_IED createVehicle position _sb;
						...
						if ((_sb distance _enemy) < 40) then
						{
							...
							while {alive _sb} do
							{
								...
								if (_sound == 1) then
								{
									[[[netid _sb,_sb], format ["suicide%1", (floor random 4)+1]], "GTOM_fnc_globalSay3D", true, false] spawn BIS_fnc_MP;
									_sound = 0
								};
