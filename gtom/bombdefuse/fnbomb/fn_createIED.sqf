/*===================================================================GTOM_fnc_createIED=================================================================================
 Create an IED mechnic (should run on server only).
	...
	[_this,_trapvolume,_IEDExplosionType,_IEDDisarmTime,_IEDJammable,_IEDTriggerType,_trapdistance,_iedside] spawn GTOM_fnc_createIED
 ...
 Params:
 	...
 	_IEDJammable		BOOLEAN - true - if jammer vheicle (defined in GTOM_IEDJammerVehicles) can jame this IED false if not
...
if (isNull _fakeIed) exitWith {diag_log "Error: GTOM_fnc_createIED object null "};
...
_fnc_iedHandle = {
	...
	while {alive _fakeIed
		...
		if !(_IEDTriggerType in [2,4]) then	{
			...
			_arrayECM = _pos nearObjects (missionNamespace getVariable ["GTOM_iedJammeDistance",80]);
			_arrayECM = _arrayECM select {(_x getvariable ["GTOM_ECM",false])};
	...
	if (!isnil "_dummyMarker") then {
		[2,compile format ["deletemarkerlocal '%1';",_dummyMarker]] remoteExec ["GTOM_fnc_globalExecute",0];
	};
	...
	switch (_IEDExplosionType) do {
		case 0:	{
			_IedExplosion = GTOM_fnc_IedDeadlyExplosion;
		};

		case 1: {
		   _IedExplosion = GTOM_fnc_IedDisablingExplosion;
		};

		case 2: {
		   _IedExplosion = GTOM_fnc_IedFakeExplosion;
		};
	...
	//Delete helper
	sleep 2;
	[_dummy] spawn GTOM_fnc_deleteHelper;
...
_dummy = createVehicle [GTOM_dummy, _pos, [], 0, "NONE"];

//If it is a mini-game
if (_IEDTriggerType >=3) then {
	_dummy setVariable ["GTOM_isIEDMiniGame",true,true];
} else {
	if (GTOM_isACE) then {
...
_dummy addEventHandler ["handleDamage",{_this call GTOM_fnc_iedHit;0}];
...
_dummy setvariable ["GTOM_disarmTime", _IEDDisarmTime  , true];
...
[_dummy,"ACE SelfInteraction to disarm"] remoteExec ["GTOM_fnc_createHelper",2];
...
{
	//[[getpos _fakeIed , (_fakeIed getVariable ["syncedObject", [0,0,0]])],"GTOM_fnc_iedSync",false,false] call BIS_fnc_MP;
	[getpos _fakeIed , (_fakeIed getVariable ["syncedObject", [0,0,0]])] remoteExec ["GTOM_fnc_iedSync",2];
};
