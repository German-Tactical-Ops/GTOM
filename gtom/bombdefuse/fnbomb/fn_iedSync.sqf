...
if (GTOM_isloading) then {waitUntil {! GTOM_isloading}};

_nearObjectsA 	= _pointA nearObjects [GTOM_dummy,50];
_nearObjectsB 	= _pointB nearObjects [GTOM_dummy,50];
...
if (count _nearObjectsA > 0 && count _nearObjectsB > 0) then {
	...
	if (str _syncedPos != "[0,0,0]") then {
		_savedObject = if (_triggerA in (curatorEditableObjects GTOM_curator)) then {_triggerA} else {attachedTo _triggerA};
