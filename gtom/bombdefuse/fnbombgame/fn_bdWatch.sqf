...
waitUntil {player getVariable ["GTOM_bombDefuseReady",false]};
...
_ls = "";
//Main loop
while {!(player getVariable ["GTOM_bombDefuseSuccess",false]) && (player getVariable ["GTOM_bombDefuseStrikes",0])<3 && time < _endTime && !(isnull _display)} do {
    _timeLeft = _endTime - time;
    _ms =[floor ((_timeLeft mod 1)*100)] call GTOM_fnc_time2string;
    _s = [floor(_timeLeft MOD 60)]call GTOM_fnc_time2string;
    _m = [floor((_timeLeft/60) MOD 60)]call GTOM_fnc_time2string;
    ...
    for "_i" from 0 to (player getVariable ["GTOM_bombDefuseStrikes",0])-1 do {
       _html = _html + " X ";
    };
    _html = _html + "</t>";
    ...
    if (_defused) then {player setVariable ["GTOM_bombDefuseSuccess",true]};
    sleep 0.1;
};

if !(player getVariable ["GTOM_bombDefuseSuccess",false]) then {
     closeDialog 0;
};