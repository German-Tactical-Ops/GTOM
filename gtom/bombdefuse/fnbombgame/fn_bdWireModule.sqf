...
for "_i" from 1 to _wires do {
    ...
    _ctrl ctrlsetText format ["%1bombDefuse\data\wireStart.paa",GTOM_path];
    _ctrl ctrlSetPosition [0.015*_ratio, (_yPos-0.005)*_ratio, 0.025*_ratio, 0.025*_ratio];
    _ctrl ctrlCommit 0;

    //Wire End
    _ctrl = _display ctrlCreate ["RscPicture", -1,_ctrlGroup];
    _ctrl ctrlsetText format ["%1bombDefuse\data\wireEnd.paa",GTOM_path];
...

{
   if (_forEachIndex == _correctWireIndex) then {
        ...
        _x ctrlAddEventHandler ["MouseButtonUp","(_this select 0) ctrlshow false; playsound 'AlarmCar'; player setVariable ['GTOM_bombDefuseStrikes',(player getVariable ['GTOM_bombDefuseStrikes',0])+1]"];
    };
} forEach _ctrls;