/*=================================================================GTOM_fnc_initBombDefuse==================================================================================
  ...
  Parameter(s):
     ...
     Example ["easy",["wires","wires","buttons","numpad","buttons"]] call GTOM_fnc_initBombDefuse"
...
_ok = createDialog "GTOM_bombDefuse";
waitUntil {dialog};
...
//Set player var
player setVariable ["GTOM_bombDefuseSuccess",false];
player setVariable ["GTOM_bombDefuseStrikes",0];
player setVariable ["GTOM_bombDefuseReady",false];
...
_control ctrlsetText format ["%1bombDefuse\data\metalModule.paa",GTOM_path];
_control ctrlSetPosition [0, 0, DEFAULT_WIDTH, DEFAULT_HEIGHT];
_control ctrlCommit 0;

[_difficulty,_ctrlGroup, count _array, _time] spawn GTOM_fnc_bdWatch;
...

{
    ...
    switch (tolower _moduleType) do {
        case "wires": {[_difficulty,_ctrlGroup,_serialNumber] spawn GTOM_fnc_bdWireModule};
        case "buttons": {[_difficulty,_ctrlGroup,_serialNumber] spawn GTOM_fnc_bdButtonsModule};
        case "numpad": {[_difficulty,_ctrlGroup,_serialNumber] spawn GTOM_fnc_bdNumpadModule};
    };
...
_ctrlBackground ctrlsetText format ["%1bombDefuse\data\metalBackground.paa",GTOM_path];
_ctrlBackground ctrlcommit 0;

//We are ready
player setVariable ["GTOM_bombDefuseReady",true];

//wait till the dialog is canceled or confirmed
waitUntil {(player getVariable ["GTOM_bombDefuseSuccess",false]) || (tolower str (findDisplay 1031983) == "no display")};

//return
player getVariable ["GTOM_bombDefuseSuccess",false];
